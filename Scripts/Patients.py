...
def load_patient_files(file_path: Path):
    con = duckdb.connect()
    df_patient = con.execute(f'select * from read_ndjson_auto("{file_path}/Patient*.ndjson", sample_size=-1, union_by_name=true, filename=true)').pl()
    con.close()
    return df_patient
...
def process_patient_basetable(df_patient:pl.DataFrame):
    ...
    if 'deceasedDateTime' not in patient_base.columns:
        patient_base = (
            patient_base
            .with_columns(
                pl.lit(None).alias('deceasedDateTime')
            ))
    ...
    if 'link' in patient_base.columns:
        patient_base_sorted = (
            patient_base
            .with_columns(
            pl.col('link').list.get(0).struct.field('other').struct.field('display').alias('id_link'),
